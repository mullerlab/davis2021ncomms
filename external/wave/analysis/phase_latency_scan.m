function [pl,modulus,cc,pv,slope,xx,yy] = phase_latency_scan( xph, wt, Fs, pixel_spacing )
% *WAVE*
%
% PHASE LATENCY SCAN       an efficient implementation of the phase latency
%                             correlation with distance calculation, for
%                             all points along the time axis in the input
%                             datacube
%
% INPUT
...
parfor ii = 1:size(pl,3)
    ...
    tmp_pl = pl(:,:,ii);
    nanfield = logical( (tmp_pl == 0) + isnan(tmp_pl) );
    tmp_pl( nanfield ) = NaN;
    smooth_pl = smoothn( tmp_pl, 'robust' );
    smooth_pl( nanfield ) = NaN;
    
    if numel( find( smooth_pl == min(smooth_pl(:)) ) ) == 1 % if unique minimum exists
        ...
        [cc(ii),pv(ii)] = corr( D, tmp_pl, 'tail', 'right' ); % h0: rho == 0, h1: rho > 0
