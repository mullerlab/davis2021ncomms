function [pl,modulus,cc,pv,slope,xx,yy] = ...
    phase_latency_scan_set_bound( xph, wt, Fs, pixel_spacing, bound_size )
% *WAVE*
%
% PHASE LATENCY SCAN SET BOUND       
%
%
%
% INPUT
...
parfor ii = 1:size(pl,3)
    ...
    if numel( find( smooth_pl == min(smooth_pl(:)) ) ) == 1 % if unique minimum exists
        ...
        % make specific bound mask
        tmp_pl( nanfield ) = NaN;
        tmp_pl( D > (bound_size/pixel_spacing) ) = NaN;
