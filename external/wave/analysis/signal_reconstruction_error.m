function [epsilon_avg,epsilon_std] = signal_reconstruction_error( xo, xn )
% *WAVE*
%
% SIGNAL RECONSTRUCTION ERROR    returns an estimate of the signal
...
assert( ndims(xo) == 3, 'datacube input required, original signal' );
assert( ndims(xn) == 3, 'datacube input required, new signal' );
assert( isequal( size(xo), size(xn) ), 'datacubes must be the same size' );
